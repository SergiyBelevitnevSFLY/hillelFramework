...
//        assertThat(listOfUsers).usingRecursiveComparison().contains(new utils.User("0000", "HillelUser", 567)).usingRecursiveComparison();
...
//
//        );
//        assertThat(listOfUsers).last().usingRecursiveComparison().isEqualTo(new utils.User("0000", "HillelUser", 567));
//
...
//
//        assertThat(listOfUsers.stream().map(utils.User::getId).filter(id -> id > 150).collect(Collectors.toList())).allMatch(id -> id > 344);
//
    }
}
